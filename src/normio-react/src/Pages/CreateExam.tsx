...
const AddHostSection: React.FC<{examId: string}> = ({ examId }) => {
    ...
    const onSubmit = useCallback(async ({ name }) => {
        await NormioApi.addHost(examId, name)
        setAdded(true)
        reset()
    }, [examId, setAdded])
    return (
        <div className={"card-text text-center p-2"}>
            ...
            <button className={"btn btn-light btn-block"} data-toggle="modal" data-target={`#${modalId}`}>네, 지금 등록할게요.</button>
            <div className="modal fade" id={modalId} tabIndex={-1} role="dialog" aria-labelledby="addHostModal" aria-hidden="true">
                        ...
                        <form onSubmit={handleSubmit(onSubmit)}>
                            <div className="modal-body">
                                <div className={"form-group"}>
                                    <input className={"form-control"} type="text" name={"name"} placeholder={"호스트 이름"} ref={register} />
                                </div>
                            </div>
                            <div className="modal-footer">
                                <button type="button" className="btn btn-secondary" data-dismiss="modal">닫기</button>
                                <button type="submit" className={`btn ${added ? "btn-success" : "btn-primary"}`}>호스트 추가</button>
                            </div>
                        </form>
                    </div>
                </div>
...
const AddStudentSection: React.FC<{examId: string}> = ({ examId }) => {
    ...
    const studentId = useState<string | null>(null)
    const [added, setAdded] = useState(false)
    const { register, handleSubmit, reset } = useForm()

    const onSubmit = useCallback(async ({ name }) => {
        await NormioApi.addStudent(examId, name)
        setAdded(true)
        reset()
    }, [examId, setAdded])
    return (
                ...
                <div className="modal-dialog" role="document">
                    <div className="modal-content">
                        ...
                        <form onSubmit={handleSubmit(onSubmit)}>
                            <div className="modal-body">
                                <div className={"form-group"}>
                                    <input className={"form-control"} type="text" name={"name"} placeholder={"학생 이름"} ref={register} />
                                </div>
                            </div>
                            <div className="modal-footer">
                                <button type="button" className="btn btn-secondary" data-dismiss="modal">닫기</button>
                                <button type="submit" className={`btn ${added ? "btn-success" : "btn-primary"}`}>학생 추가</button>
                            </div>
                        </form>
                    </div>
                </div>
...
const ChangeExamConfigSection: React.FC<{examId: string, title: string}> = ({ examId, title }) => {
    ...
    return (
        <div className={"card-text text-center p-2"}>
            ...
            <button className={"btn btn-light btn-block"} data-toggle="modal" data-target={`#${modalId}`}>네, 수정하고 싶습니다.</button>
            <div className="modal fade" id={modalId} tabIndex={-1} role="dialog" aria-labelledby={modalId} aria-hidden="true">
                <div className="modal-dialog" role="document">
                    <div className="modal-content">
                        <div className="modal-header">
                        ...
                        <form onSubmit={handleSubmit(onSubmit)}>
                            <div className="modal-body">
                                <div className={"form-group"}>
                                    <input className={"form-control"} type="text" name={"newTitle"} placeholder={"새로운 시험 명"} ref={register} />
                                </div>
                            </div>
                            <div className="modal-footer">
                                <button type="button" className="btn btn-secondary" data-dismiss="modal">닫기</button>
                                <button type="submit" className={`btn ${added ? "btn-success" : "btn-primary"}`}>설정 저장</button>
                            </div>
                        </form>
                    </div>
                </div>
